...
public class ProcessScheduler {
    ...
    public ProcessScheduler(MemoryManager memoryManager, FileSystem fileSystem, TextArea outputArea) {
    ...
    public synchronized void addProcess(String name, String content) {
        if (isProcessRunningOrQueued(name)) {
            logToUI("Proces " + name + " je već pokrenut ili u redu za čekanje.");
            return;
        }

        Process process = new Process(name, content, assembler, this, outputArea);
        int requiredBlocks = process.getRequiredBlocks();

        if (!fileSystem.hasSufficientFreeBlocks(requiredBlocks)) {
            logToUI("Nema dovoljno slobodnih blokova za proces " + name);
            return;
        }
        ...
        logToUI("Proces " + name + " je dodan u red čekanja sa početnom adresom " + process.getStartAddress());
        executeNextProcess();
    }
    ...
    public synchronized void executeNextProcess() {
        if (runningProcess == null && !processQueue.isEmpty()) {
            ...
            if (startAddress != -1) {
                logToUI("Proces " + runningProcess.getProcessName() + " je započet sa početnom adresom " + runningProcess.getStartAddress());
                runningProcess.start();
            } else {
                logToUI("Nedovoljno memorije za pokretanje procesa " + runningProcess.getProcessName());
    ...
    public synchronized String unblockProcess(String name) {
        for (Process process : blockedQueue) {
            if (process.getProcessName().equals(name)) {
                ...
                Process newProcess = new Process(process.getProcessName(), process.getContent(), assembler, this, outputArea);
    ...
    public synchronized void processFinished() {
        if (runningProcess != null) {
            logToUI("Proces " + runningProcess.getProcessName() + " je završen.");
            memoryManager.freeMemory(runningProcess.getStartAddress());
            fileSystem.freeBlocks(runningProcess.getBlockIndices());
            runningProcess = null;
            executeNextProcess();
